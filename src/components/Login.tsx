...
const Login = () => {
  const login = () => {
    ...
    if (window.location.href.includes('authz.constantcontact.com')) {
    ...
    const url = `https://authz.constantcontact.com/oauth2/default/v1/authorize?client_id=${clientId}&userId=YOUR_USER_ID&redirect_uri=${encodeURIComponent(
      redirectUri
    )}&response_type=code&scope=contact_data%20offline_access&state=${state}`;
